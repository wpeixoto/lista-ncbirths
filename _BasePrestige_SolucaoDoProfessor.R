...
# diferença entre tratar e não tratar nulos
...
# começo pelos significativos, iniciando com o mais significativo
...
# variacao de women não é significativa
fit3 <- lm(prestige ~ education + income + women, data = prestige)
summary(fit3)
anova(fit1, fit2, fit3)

# variação de type é significativa mas os coeficientes, não.
fit4 <- lm(prestige ~ education + income + type, data = prestige)
summary(fit4)
anova(fit1, fit2, fit4)

# e type tem alto fator de inflação: não compensa
vif(fit4) 
# além disso, o R2 é praticamente o mesmo
...
vif(fit5) # não compensa a interação

# então fechamos o modelo em fit2: prestige ~ education + income

# temos um bom modelo que explica o prestígio, mas,
# suponha que o estudo é sobre perstigio em função do gênero: fit2 não é útil, voltamos ao fit3
vif(fit3)
...
summary(fit6)
plot(fit6) # a decisão de classificar newsboy como prof deu a ele potencial de alavancagem
# acrescentamos type
...
# escolha: fit7
#######################
# considerando que existe alguma variação quando type varia de bc para wc
# e que os resíduos se aproximam de uma distribuição normal quando a 
# variável type é adicionada
...
# predição com a função predict
predict(fit7,list(education=10,women=50,type="wc"),interval = "conf")


######################################
# outra possibilidade: income na saída
######################################
